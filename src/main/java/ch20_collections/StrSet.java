...
/*
    ...
    (수학에서의 집합개념을 Collection으로 구현한 것이 Set에 해당)
    그래서 특성의 차이로 인해 List -> Set / Set -> List로의 형변환이 매우 중요.
    List를 통해 전체 설문을 받고, Set을 통해 중복을 제거하여 후보군들만 남기는 등의
    응용이 가능하기 때문에, 혹은 Set을 List로 바꿔 .get(indexNumber)를 통해 조회를 하기 위해
    서로 간의 형변환이 매우 중요합니다.
 */
public class StrSet {
    public static void main(String[] args) {
        ...
        // 형변환이 중요하다고 했으니 List도 하나 만들어두겠습니다.
...
//        }                             // Set의 경우 인덱스가 없기 때문에 반복문작성이 애매하다.

        System.out.println();
        // 특정element를 추출 -> List로의 형변환을 할겁니다.
        ...
        //새로운 Set을 선언해서 다시 strList1에 있는 부분을 다 더해버리면 중복이 제거도니다.
