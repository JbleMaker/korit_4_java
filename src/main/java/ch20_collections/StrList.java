...
public class StrList {
    public static void main(String[] args) {
        // 비어있는 List 생성방법
        List<String> strList1 = new ArrayList<>();          // 리스트의 경우 인덱스넘버를 고정 시킬 필요가 없음
        // 배열과의 차이점도 볼 필요가 있습니다.
        String[] strArray1 = new String[2];                  // 배열은 인덱스넘버를 고정시켜야 한다.
        ...
        // 출력
        System.out.println(strArray1);                          // 참조자료형이라서 주소값이 출력된다.
        System.out.println(Arrays.toString(strArray1));         // 배열은 출력하기 위해 static method를 경유해야한다.
        System.out.println(strList1);                           //

        // 특정 element의 포함 여부 -> contains() 메서드를 사용 -> return boolean
        String searchElem1 = "python";                          // 하드 코딩을 피하기 위해 변수 대입
        boolean contains1 = strList1.contains(searchElem1);     // elem이란 표현은 element의 축약어로 자주 사용.
        System.out.println(searchElem1 + "의 포함여부 : " + contains1);

        String searchElem2 = "py";                          // 하드 코딩을 피하기 위해 변수 대입
        boolean contains2 = strList1.contains(searchElem2);     // elem이란 표현은 element의 축약어로 자주 사용.
        System.out.println(searchElem2 + "의 포함여부 : " + contains2);
        ...
        // 마찬가지로 .remove()역시 element 전체가 일치하지는지를 확인하기 때문에
        // elemenet의 일부만 가지고 삭제하는 것이 불가능합니다. (일부로 element 전체 삭제 불가능,
        ...
        // python 언어 java언어 JavaScript언어 C#로 출력될 수 있도록 작성하시오.
        // 참고 : strList1.length는 안된다. -> List는 size를 사용한다.
        System.out.println();

        for(int i = 0; i < strList1.size(); i++){       // 일반 for문 사용
            System.out.print(strList1.get(i) + "언어 ");
        }
        
        System.out.println();
        for(String language : strList1){                // 향상된 for문 사용
            System.out.print(language + "언어 ");
        }
        // 이상의 코드에서 알 수 있는 점은 
        // 배열과는 달리 한계값을 출력하기 위해서 리스트명.size()를 사용
        // 출력할 때도 리스트명.get(i)를 썻다는 점
        // 배열의 경우는 배열명.length
        // 배열명[i]를 사용.
    }
}
