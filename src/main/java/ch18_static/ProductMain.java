...
//        System.out.println(Arrays.toString());    // 이상의 방식의 호출을 이미 사용+했었다.

        System.out.println(Product.getCount());     //클래스명.메서드명()을 통한 getter
        Product product = new Product();
        System.out.println("count : " + Product.getCount());
        System.out.println("instanceCount : " + product.getInstanceCount());
        System.out.println();
        Product product1 = new Product();
        System.out.println("count : " + Product.getCount());        // 정적 메서드 호출
        System.out.println("instanceCount : " + product1.getInstanceCount());   // 일반 메서드 호출
        ...
        // 결과적으로 인스턴스 변수는 객체에 종속되어 있기 때문에 객체 생성시에 0에서 1로 증가되는 것 밖에
        // 결과가 나오지 않지만, 정적 변수인  count의 경우는 객체가 생성될 때 마다 count++ 가 적용되어
        // 객체의 수 만큼 count가 갱신된다는 점을 확인할 수 있다.
        // 즉, 인스턴스 변수 -> 객체에 종속 / 정적변수 -> 클래스에 종속
