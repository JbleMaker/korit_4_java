...
public class Main {
    public static void main(String[] args) {
        ...
        phoneFactory1.produce("아이폰15 PRO");      // 재정의된 메서드 #1 - 추상 메서드
        ...
        // 메서드를 작성할때는 세미콜론(;)이 필요하지 않음.
        // 익명 클래스
        // 추상 메서드를 main단계에서 구현. 그리고 객체 생성을 한 상태.
        // 그렇다면 일반적인 클래스를 기준으로 봤을 때 메서드를 정의한 부분까지 작성했다고 볼 수 있다.
        // factory1에 new Factory(){}를 대입한 것이기 때문에 세미콜론(;)이 있어야된다.

        factory1.setName("가전 제품 공장");       // 이건 메서드 호출에 해당.
        factory1.produce("백색 가전");    // 동일.

        /*
            이상의 개념은 추상 클래스의 개념에서 벗어납니다.(객체 생성이 불가능하다고 했는데
            Factory 클래스의 객체가 생성되었다는 점에서).
            Java17인지 11인지에서 도입된 익명 클래스라는 개념인데, 재사용하지 않고 한 번만 쓰고 치울거라면
            (즉, 이 Main 클래스에서만 사용하고 다른 클래스에서는 사용하지 않을거라면), 사용하기에 좋다.
            보조수단으로 생각
         */
