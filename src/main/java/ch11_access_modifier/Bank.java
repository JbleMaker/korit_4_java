...
public class Bank {
    ...
    public void setBalance(int balance) { // 계좌잔액 0이상
        if(balance < 1){
    ...
    public void setPinNumber(int pinNumber) { // 계좌 비밀번호 0~9999
        this.pinNumber = pinNumber;
    ...
    public void deposit(int amount, int inputPin){ // 계좌 입금
    ...
    public void withdraw(int amount, int inputPin){ // 계좌 출금
        if(inputPin == this.pinNumber){ // 비밀번호 확인
            if(amount > 0){        // 잔액이 0원 이상인지
                if(balance - amount < 0){     // 출금후 잔약이 0원 이하인지
    ...
    public void showAccountInfo() { //잔액 확인
