...
//        animalData3.printData();
        /*
            25번 라인에서 오류가 생긴다는 점을 확인
            AnimalData 클래스의 .printData() 메서드를 호출하는 과정에서
            Car 객체(AnimalData의 필드인)를 강제로 Animal클래스로 형변환하려고 했기 때문에
            발생하는 예외 인데 결국 필드에 Car객체가 대입은 되지만 메서드는 호출되지 않는다는 점을 확인

            그렇다면 애초에 Car객체가 필드에 대입되지 못하도록 설정할 필요성이 대두되었는데,
            이 것이 와일드 카드 개념의 의의.
         */

        // Main에 메서드 정의를 했으니까 Main 객체를 생성.
        Main main = new Main();
        ...
        // 이상의 코드를 적성했을 때 생성자를 통해서 객체 생성을 하지 않았다는 점,
        // 저희가 직접 getAnima() 메서드를 정의하지 않았다면 어떤 클래스의 객체가 argument로 들어갔는지
